...
int main()
{
	...
	cl_mem c_mem_obj = clCreateBuffer(context,CL_MEM_WRITE_ONLY,index*sizeof(int),NULL,&ret);


	ret = clEnqueueWriteBuffer(queue,a_mem_obj,CL_TRUE,0,n*sizeof(char),a,0,NULL,NULL);
	ret = clEnqueueWriteBuffer(queue,c_mem_obj,CL_TRUE,0,index*sizeof(int),len,0,NULL,NULL);
